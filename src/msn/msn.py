...
class switchboard_session(messaging_support, basic.LineReceiver):
    ...
    def __init__(self, myhandle, challenge, mode):
    ...
    def handle_bye(self, args):
        self.handler.handle_bye(args[1])
    ...
    def rawDataReceived(self, data):
        ...
        if len(self.msgbuffer) >= self.msglen:
            ...
            self.handler.message_received(self.currentmsghandle, 
                                          self.msgbuffer[:self.msglen])
            self.setLineMode(self.msgbuffer[self.msglen:])
            ## clear buffer
            self.msgbuffer = ""
            self.msglen = -1
            self.currentmsghandle = ""

class messenger(messaging_support, basic.LineReceiver):
    DS_MODE = 0  # dispatch server mode
    NS_MODE = 1  # nameserver server mode
    # would BS mode make sense?

    def __init__(self, handle, password, mode=DS_MODE):
        messaging_support.__init__(self)
        self.handle = handle
    ...
    def close(self):
        """ close the connection to either nameserver or switchboardserver """
    ...
    def connectionMade(self):
        ...
        if self.mode == messenger.DS_MODE:
            print "DS connection"
            self.ver()
            # self.inf()
        else:
    ...
    def rawDataReceived(self, data):
        ...
        if len(self.msgbuffer) >= self.msglen:
            ...
            ##
            ## Message is complete, generate event.
            self.handler.message_received(self.msgbuffer[:self.msglen])
            self.setLineMode(self.msgbuffer[self.msglen:])
            ## clear buffer
            self.msgbuffer = ""
            self.msglen = -1
    ...
    def handle_xfr(self, args):
        ...
        if args[2] == 'NS':
            ...
            (h,p) = split(args[3], ":")
            (host, port) = (h, int(p))
            self.handler.xfr_ns(host, port)
        elif args[2] == 'SB':
            ##
            ## xfr to switchboard -- create sb session
            (cmd, trid, type, hostip, sp, challenge) = args
            (host, port) = split(hostip, ":")
            self.handler.xfr_sb(int(trid), host, int(port), challenge)
    ...
    def handle_lst(self, args):
        ...
        if int(args[5]) > 0: #  and len(args) == 8:
            ...
            if args[4] == args[5]:
                self.handler.list_complete(args[2])
    ...
    def handle_rng(self, args): 
        ...
        self.handler.ring(host, port, handle, friendly, challenge, sessionid)

    def handle_nln(self, args):
        """ handle online state change """
        self.handler.handle_onlinechange(args[1], args[2], args[3])

    def handle_iln(self, args):
        """ handle initial online state notification """
        self.handler.handle_online(args[2], args[3], args[4])

    def handle_fln(self, args):
        """ handle offline state change """
        self.handler.handle_offline(args[1])
